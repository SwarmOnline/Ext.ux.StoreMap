...
Ext.define('Ext.ux.StoreMap', {
	...
	refresh: function(){
		...
		if (!this.getMap()) {
			this.un('maprender', 'refresh', this);
			this.on('maprender', 'refresh', this, { delay: 150, single: true });
		} else {
	...
	updateCurrentLocation: function(location, oldLocation){
		...
		if (!this.getMap()) {
			this.un('maprender', 'doUpdateCurrentLocation', this);
			this.on('maprender', 'doUpdateCurrentLocation', this, { delay: 150, single: true, args: [location] });
		} else {
	 ...
	 * This logic is extracted out so it can be attached to a 'maprender' event if the map isn't ready
...
var hasLoc = location && location.lat && location.lng;
	...
	updateCentreOnCurrentLocation: function(val){
		if(val === false){
			if (!this.getMap()) {
				this.un('maprender', 'doCentreOnDefaultLocation', this);
				this.on('maprender', 'doCentreOnDefaultLocation', this, { delay: 150, single: true });
			} else {
	...
	updateMarker: function(record){
		...
		if(marker){
			marker.setPosition(this.getGMLocation(lat, lng));
